...
Обновлено: 3 сентября 2026 года. База исходников: `bbc1f9bab00e8255e8d73b138607126c595dde06`.
...
## Текущее состояние: проверка 3 сентября, 11:29 МСК
...
Это ещё не опубликованный релиз. После повторного входа пользователя GitHub
...
установкой, на которой выполняется native-приёмка. Ссылки на скачивание
в Studio пока не включались. Кандидат не устанавливался в основной профиль:
проверка чистой установки из релиза остаётся отложенной.

## Оставшаяся работа

Текущий объём: завершить полную live-приёмку Hermes, собрать на сервере и
опубликовать финальные пакеты, обновить страницу подключения, закрыть остальные
итоговые проверки и документацию. По уточнению пользователя от 3 сентября сценарий
и его приёмка через Hermes должны выполняться через подписку ChatGPT, уже
подключённую к Hermes на этой машине. Это явное исключение из общего ограничения
моделями конфига Dock; серверные функции Dock сохраняют свои настроенные модели.
Прежние таймауты относились к запускам через OpenRouter и не подтверждают состояние
подключения Hermes к ChatGPT. Авторизация GitHub восстановлена и проверена;
публикация ожидает завершения текущей приёмки Hermes.
...
Выпуск по оставшемуся объёму пока не завершён.
