...
Перед **следующим клиентским выпуском** учесть отдельный дефект состава тестов:
`client/test/landing.test.mjs` импортирует `landing/instructions.mjs` и
`landing/release.json`, а упаковщик клиента не включает `landing/`. В изолированной
структуре клиентского снимка тест падает с `ERR_MODULE_NOT_FOUND`. Это подтверждено
при актуализации документации; исправление кода в эту задачу не входило. Нужно
разделить тесты сайта и распространяемого клиента или обеспечить их зависимости.
