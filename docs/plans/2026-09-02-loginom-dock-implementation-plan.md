...
Эти задачи остаются открытыми, но исключены из текущего объёма работ.
Полная приёмка Hermes и публикация предварительного выпуска завершены.
Дополнение от 3 сентября: страницу подключения вынести в отдельный русскоязычный
лендинг на `loginom-dock.duckdns.org`, с цветами Loginom, подробной установкой
плагинов и описанием работы. Подтверждённые результаты — в журнале реализации.
...
- [x] Обновить README, название и ссылки веб-интерфейса, иконки, описания MCP,
...
- [x] Проверить DOM, реальные координатные drag, clipboard, dialogs, загрузку
  файлов, скриншоты и длительные ожидания Loginom.
- [x] Выполнить ранний прототип на обоих агентах: получить skill и источники через
...
**Точка входа:** [русскоязычный лендинг](https://loginom-dock.duckdns.org/#install)
с выбором Codex/Hermes и macOS/Linux. Прежняя страница Studio перенаправляет сюда.
Включение в публичные каталоги OpenAI или Hermes не является условием первого релиза.

- [x] Подготовить два native-пакета и общий локальный runtime. Каждый пакет
...
- [x] Выпускать проверенные артефакты для заявленных macOS/Linux платформ с
...
- [ ] Для Hermes использовать штатную установку native plugin из подкаталога
  ...
  Механизм реализован и проверен с локальным Git-fixture; проверка установки
  из опубликованного GitHub-релиза отложена пользователем 3 сентября.
- [x] Создать установщик/мастер Dock: проверить агента и ОС, подготовить среду
...
- [x] Добавить инструкции установки, обновления, восстановления и диагностики;
...
- [x] Проверить сервер без изменений: ОС, ресурсы, Docker, порты, существующие
  сервисы и каталоги. Первичная проверка и последующее публичное развёртывание
  подтверждены; текущая конфигурация и пути описаны в `../loginom-dock/operations.md`.
...
- [ ] При изменениях CI синхронно обновлять каноническую документацию и её
  ...
  Условие для будущих изменений: в выпуске `loginom-dock@0.1.0-rc.1` CI не менялся,
  новые копии документации CI в OpenViking не публиковались.
